...
Documents open **with the editor**: a palette to add/remove objects (Duplicate button or Ctrl+D copies the selected object's spec and initial properties, never its tracks), an inspector that edits initial properties (auto-generated widgets per property type), and a dope sheet in the transport panel — per-track keyframe lanes with draggable diamonds, double-click to insert, a scrubbing ruler, and per-keyframe easing/value editing. Add keyframes with the per-track `+` button (at the playhead) or by double-clicking a lane; note a track needs **two** keyframes to animate — before its first keyframe a property holds that keyframe's value rather than blending from the inspector's initial value, and once tracked, the track owns the property from t=0. Everything saves back to the RON file. "+ New scene" in the library creates a fresh document (rename it any time via the name field in the editor, which renames the underlying file) — build a scene, animate it, hit Save, then Export, without writing any Rust. Doc entries in the library also have a Delete button (two-click confirm) that removes the underlying file. Documents can store export defaults (resolution/fps/output) via the export screen’s "Save as scene defaults" — the export form and the non-interactive CLI both honor them.
