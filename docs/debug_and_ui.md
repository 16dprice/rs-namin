...
Preview mode (P key or the app bar's Preview button) hides every panel and overlay and plays the scene back exactly as export renders it: the document camera driven by camera tracks, rendered through `OffscreenRenderer` and letterboxed to the window at the 16:9 design aspect. The orbit camera is untouched, so Esc (or P) returns to the editing view as it was. Transport keys (space, arrows) stay live; the panel-toggle and snap keys deliberately don't. See `ViewerMode::preview_frame` in `src/viewer.rs`.
