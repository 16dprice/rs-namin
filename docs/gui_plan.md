...
- **M2.1** ✅ **DONE** — `Easing` enum replaces the bare fn pointer everywhere (28 named
  ...
  LSystem joined the spec in Jul 2026 (axiom/rules are pure data; the editor gets a rules UI), and Sprite in Aug 2026 (image as a file path, texture resolved lazily at draw time through scene::texture_cache — spawn stays GL-free/infallible; missing files draw a placeholder). Turtle is deliberately NOT a spec: in documents it is composed — a Sprite with position/rotation bound to an L-system's pen_position/pen_angle outputs. Still not representable: custom fonts/LaTeX — they get
