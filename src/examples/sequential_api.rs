...
pub fn build() -> (Scene, Timeline, Camera) {
    ...
    let backdrop = sb.add(Rectangle::new(
        vec3(0.0, 2.0, -0.1),
        vec2(12.0, 6.0),
        DARKGRAY,
    ));
    ...
    sb.animate_camera("position", |tb| {
        tb.keyframe_with_easing(
            0.0,
            AnimValue::Vec3(vec3(0.0, 2.5, 10.0)),
            easing::sine_in_out,
        )
    ...
    sb.animate(&ground, "color", |tb| {
        tb.keyframe_with_easing(
            0.0,
            AnimValue::Vec4(vec4(1.0, 1.0, 1.0, 0.0)),
            easing::linear,
        )
    ...
    sb.animate(&backdrop, "color", |tb| {
        tb.keyframe_with_easing(
            0.0,
            AnimValue::Vec4(vec4(bg.x, bg.y, bg.z, 0.0)),
            easing::sine_out,
        )
